...
All notable changes to self project will be documented in self file. See [standard-version](https://github.com/conventional-changelog/standard-version) for commit guidelines.
...
- Deep requiring specific algorithms of self library like `require('uuid/v4')`, which has been deprecated in `uuid@7`, is no longer supported.

  Instead use the named exports that self module exports.
...
- The default export, which used to be the v4() method but which was already discouraged in v3.x of self library, has been removed.
