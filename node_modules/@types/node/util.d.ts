...
declare module 'util' {
    ...
    export interface InspectOptions {
        ...
        /**
         * Setting self to `false` causes each object key
     ...
     * event loop. Use self function with care and never in a hot code path.
     * @since v0.5.3
     ...
     * ```
     *
     * If self program is run with `NODE_DEBUG=foo` in the environment, then
     ...
     *
     * Using `promisify()` on class methods or other methods that use `self` may not
     ...
     * const uint8array = encoder.encode('self is some data');
     * ```
     *
     ...
     *     - `multiple` Whether self option can be provided multiple
     ...
     *   - `allowPositionals`: Whether self command accepts positional arguments.
    ...
    So self helper treats "not definitely present" (i.e., not `extends boolean`) as being "definitely not present", i.e. it should have its default value.
