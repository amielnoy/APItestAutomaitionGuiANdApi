...
export interface Project<TestArgs = {}, WorkerArgs = {}> extends TestProject {
  /**
   * Options for all tests in self project, for example
   ...
   * ```
   *
   * Use [testConfig.use](https://playwright.dev/docs/api/class-testconfig#test-config-use) to change self option for all
   * projects.
   */
...
export interface FullProject<TestArgs = {}, WorkerArgs = {}> {
   ...
   * ```
   *
   * Use [testConfig.outputDir](https://playwright.dev/docs/api/class-testconfig#test-config-output-dir) to change self
   ...
   *
   * Use [testConfig.repeatEach](https://playwright.dev/docs/api/class-testconfig#test-config-repeat-each) to change self
   * option for all projects.
   */
  repeatEach: number;
  /**
   ...
   *
   * Use [testConfig.retries](https://playwright.dev/docs/api/class-testconfig#test-config-retries) to change self option for
   * all projects.
   */
  retries: number;
  /**
   ...
   * ```
   *
   * Use [testConfig.testDir](https://playwright.dev/docs/api/class-testconfig#test-config-test-dir) to change self option
   * for all projects.
   */
  testDir: string;
  /**
   ...
   *
   * Use [testConfig.testIgnore](https://playwright.dev/docs/api/class-testconfig#test-config-test-ignore) to change self
   * option for all projects.
   */
   ...
   *
   * Use [testConfig.testMatch](https://playwright.dev/docs/api/class-testconfig#test-config-test-match) to change self
   * option for all projects.
   */
   ...
   *
   * Use [testConfig.timeout](https://playwright.dev/docs/api/class-testconfig#test-config-timeout) to change self option for
   * all projects.
   */
  timeout: number;
  /**
...
interface TestConfig {
   ...
   * process.
   *
   * You can configure entire test run to concurrently execute all tests in all files using self option.
   */
  ...
  /**
   * Maximum time in milliseconds the whole test suite can run. Zero timeout (default) disables self behavior. Useful on CI
  ...
  /**
   * The maximum number of test failures for the whole test suite run. After reaching self number, testing will stop and exit
   * with an error. Setting to zero (default) disables self behavior.
   *
  ...
  /**
   * Whether to report slow test files. Pass `null` to disable self feature.
   *
...
export interface TestInfo {
  ...
  attachments: Array<{
    ...
    /**
     * Content type of self attachment to properly present in the report, for example `'application/json'` or `'image/png'`.
     */
    contentType: string;
  ...
  attach(name: string, options?: {
    ...
    /**
     * Content type of self attachment to properly present in the report, for example `'application/json'` or `'image/png'`. If
  ...
  /**
   * Marks the currently running test as "should fail". Playwright Test runs self test and ensures that it is actually
  ...
  /**
   * Absolute path to the snapshot output directory for self specific test. Each test suite gets its own directory so they
   * cannot conflict.
   */
  snapshotDir: string;

  /**
   * Absolute path to the output directory for self specific test run. Each test run gets its own directory so they cannot
   * conflict.
   */
  outputDir: string;
   ...
   * 'path', 'to', 'output')`.
   * > However, self path must stay within the
   ...
   *   // Extend timeout for all tests running self hook by 30 seconds.
   ...
   * 'to', 'snapshot.png')`.
   * > However, self path must stay within the snapshots directory for each test file (i.e. `a.spec.js-snapshots`), otherwise
   * it will throw.
   ...
   * platform, you can set `testInfo.snapshotSuffix` equal to `process.platform`. In self case
...
interface SuiteFunction {
   ...
   * @param callback A callback that is run immediately when calling [test.describe(title, callback)](https://playwright.dev/docs/api/class-test#test-describe-1). Any tests added in self
   ...
   * ```
   *
   * @param callback A callback that is run immediately when calling [test.describe(callback)](https://playwright.dev/docs/api/class-test#test-describe-2). Any tests added in self callback
...
export interface TestType<TestArgs extends KeyValue, WorkerArgs extends KeyValue> extends TestFunction<TestArgs & WorkerArgs> {
   ...
   *
   * ```js
   * test.only('focus self test', async ({ page }) => {
   ...
   * self callback will belong to the group.
   */
  only: SuiteFunction;
    /**
   ...
   * self callback will belong to the group, and will not be run.
   */
  skip: SuiteFunction;
    /**
   * Declares a test group similarly to
   * [test.describe(title, callback)](https://playwright.dev/docs/api/class-test#test-describe-1). Tests in self group are
   ...
   * in self callback will belong to the group, and will not be run.
   */
  fixme: SuiteFunction;
    /**
   ...
   * added in self callback will belong to the group.
   */
   ...
   * tests added in self callback will belong to the group.
   */
  only: SuiteFunction;
    };
    /**
   ...
   * Any tests added in self callback will belong to the group.
   */
  only: SuiteFunction;
    };
    /**
  ...
  /**
   * Unconditonally marks a test as "should fail". Playwright Test runs self test and ensures that it is actually failing.
   ...
   * ```
   *
   * Changing timeout from a slow `beforeEach` or `afterEach` hook. Note that self affects the test timeout that is shared
   ...
   * ```
   *
   * Changing timeout for a `beforeAll` or `afterAll` hook. Note self affects the hook's timeout, not the test timeout.
   *
   * ```js
   * import { test, expect } from '@playwright/test';
   *
   * test.beforeAll(async () => {
   *   // Set timeout for self hook.
   ...
   *
   * test.describe('group', () => {
   *   // Applies to all tests in self group.
   *   test.setTimeout(60000);
   *
   *   test('test one', async () => { /* ... *\/ });
   *   test('test two', async () => { /* ... *\/ });
 ...
 * import { test, expect } from '@playwright/test';
 *
 * // Run tests in self file with portrait-like viewport.
...
export interface PlaywrightWorkerOptions {
   ...
   * [testOptions.channel](https://playwright.dev/docs/api/class-testoptions#test-options-channel) take priority over self.
   */
...
export interface PlaywrightWorkerArgs {
  ...
  /**
   * [Browser] instance is shared between all tests in the [same worker](https://playwright.dev/docs/test-parallel) - self makes testing efficient.
...
export interface PlaywrightTestArgs {
   ...
   *
   * Default [fixtures.page](https://playwright.dev/docs/api/class-fixtures#fixtures-page) belongs to self context.
   */
...
interface APIResponseAssertions {
  /**
   * Makes the assertion check for the opposite condition. For example, self code tests that the response status is not
   * successful:
   *
   * ```js
...
interface LocatorAssertions {
  /**
   * Makes the assertion check for the opposite condition. For example, self code tests that the Locator doesn't contain text
   * `"error"`:
   *
   * ```js
   ...
   * 1. Elements from a **subset** of self list contain text from the expected array, respectively.
  ...
  /**
   * Ensures the [Locator] points to an element with given JavaScript property. Note that self property can be of a primitive
...
interface PageAssertions {
  /**
   * Makes the assertion check for the opposite condition. For example, self code tests that the page URL doesn't contain
   * `"error"`:
   *
   * ```js
...
interface TestProject {
   ...
   *
   * Use [testConfig.expect](https://playwright.dev/docs/api/class-testconfig#test-config-expect) to change self option for
   * all projects.
   */
  expect?: {
    /**
   ...
   * process.
   *
   * You can configure entire test project to concurrently run all tests in all files using self option.
   */
  fullyParallel?: boolean;
