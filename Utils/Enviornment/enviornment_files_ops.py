...
def get_envvars(env_file='enviornment/.build_details_env', set_environ=True, ignore_not_found_error=False,
                exclude_override=()):
    """
        ...
        with open(env_file) as f:
            for line in f:
                ...
                if key in exclude_override:
                    env_vars_dictionary[key] = os.getenv(key)
                else:
                    env_vars_dictionary[key] = value
    except FileNotFoundError:
        if not ignore_not_found_error:
            raise

    return env_vars_dictionary
