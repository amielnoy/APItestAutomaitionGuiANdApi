...
class TestPasswordLessFileAnalyzer:
    ...
    def passwordless_file_upload_and_analyze(self):
        ...
        expect(self_analyze_file_page.scan_upload_area).to_be_visible(timeout=30000)
        # page.pause()
        for file_detals in self.files_data:
            file_full_path = str(self.root_project.parent) + "/TestData/TestScanInputFiles/password_less/" + \
                             file_detals[0].strip()
            ...
            if os.path.isfile(file_full_path):
                ...
                print("is_running_scan_by_id_of_item=")
    ...
    def passwordless_file_scan_all_files(self):
        # verify scan results
        for file_details in self.files_data:
            ...
            # time.sleep(3)
            expect(scan_results_list_page.scan_search).to_be_visible()
            # clear_all_filters
            ...
            assert ComapreTimeStamp.is_last_scan_series_match(
                top_scan_time_stamp), "failed to find coerrect last file scan"
            scan_results_list_page.click_to_open_top_scan_result()

            Assertions.assert_boolean_value("verify correct url of scans",
                                            scan_results_list_page.verify_is_url_prefix(self.base_url + "scans"), True)
            assert scan_results_details_page.get_summary_file_name_line_text() == "File Name\n" + scanned_file_to_search.strip() \
    ...
    def assert_scan_file_details(self, file_details, scan_results_details_page, scanned_file_to_search):
        ...
        if len(uploading_user) > 0:
            Assertions.assert_value("uploading_user(scan result details)", uploading_user,
                                    ("Uploading User\n" + file_details[1].strip()))

        file_extension = scan_results_details_page.get_summary_file_extension_line_text()
        Assertions.assert_value("file_extension(docx,eml...)"
                                , file_extension
                                , "File Extension\n" + file_details[2].strip())

        mime_type = scan_results_details_page.get_summary_mime_type_line_text()
        Assertions.assert_value("Mime_type(html/txt...)", mime_type, "Mime Type\n" + file_details[3].strip())
        ...
        if file_details[5].strip() == "Malicious":
            Assertions.assert_value("Verdict(Malicious/suspicious/restricted/clean)"
                                    , scan_results_details_page.get_verdict_malicious_result()
                                    , (file_details[5].strip()))
            if scanned_file_to_search == 'mal.docx':
                Assertions.assert_value("hap_word2010(Microsoft Word 2010)"
                                        , scan_results_details_page.get_hap_word2010_text()
                                        , "Microsoft Word 2010")
                Assertions.assert_value("hap_word2010(Microsoft Word 2016)"
            ...
            Assertions.assert_value("Verdict(verdict==clean)"
                                    , scan_results_details_page.get_verdict_clean_result()
                                    , file_details[5].strip())
        Reporting.report_allure_and_logger("INFO",
                                           "*********\nFINISHED VERIFING FILE SCAN of file=" + scanned_file_to_search + " SUCCESSFULY\n******")
